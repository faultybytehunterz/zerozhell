...
class Main(base.Module):
    ...
    def do_run(self, line):
        ...
        try:
            ...
            if result:
                for wifi in result:
                    print(f"{wifi['SSID']:<30}\t {wifi['BSSID']:^18}\t {wifi['CHANNEL']:^9}\t {wifi['SIGNAL']:^9}\t {wifi['BARS']:^8}\t {wifi['SECURITY']:^18}")
        except Exception:
            print(text=f"Error: Wireless interface {self.parameters['iface']} is busy or monitor mode enabled!")
